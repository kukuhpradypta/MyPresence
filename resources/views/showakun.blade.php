                ...
                <div class="card border-0 shadow rounded">
                    <div class="card-body" id="editakun">
                        <div class="text-center mt-5">
                            ...
                            src="{{ old('foto', Storage::url('public/siswas/').$guru->Foto , Auth::guard('siswa')->user()->foto) }}"
                            @elseif(Str::length(Auth::guard('guru')->user())>0)
                            src="{{ old('foto', Storage::url('public/gurus/').$guru->Foto , Auth::guard('guru')->user()->Foto) }}"
                            @endif alt="">
                        </div>
                        <div class="container mt-3">
                            <h5>Nama :
                                @if(Str::length(Auth::guard('siswa')->user())>0)
                                {{ old('nama', Auth::guard('siswa')->user()->namasiswa) }}
                                @elseif(Str::length(Auth::guard('guru')->user())>0)
                                {{ old('nama', Auth::guard('guru')->user()->namaguru) }}
                            @endif
                            </h5>
                            <h5>Email :
                                @if(Str::length(Auth::guard('siswa')->user())>0)
                            ...
                            <a href="@if(Str::length(Auth::guard('siswa')->user())>0)
